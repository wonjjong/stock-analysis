...
type Source = {
  ...
  crawl_hour_kst: number; max_pages: number; window_hours: number; is_active: number; last_status: string; last_error: string | null;
  last_crawled_at: number | null; next_crawl_at: number; article_count: number; recent_article_count: number; run_count: number;
};
type Article = {
...
export function NewsSources() {
  ...
  async function addSource(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/news/sources", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, url, category, crawlHourKst: hour, maxPages, windowHours }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error ?? "등록에 실패했습니다.");
      setUrl(""); setName("");
      setNotice(payload.warning ? `URL은 저장됐지만 첫 수집은 실패했습니다: ${payload.warning}` : `최근 ${windowHours}시간 기사 ${payload.crawl.fetchedCount}개를 확인했습니다. 매일 ${hour}시에 다시 수집합니다.`);
      await load();
  ...
  async function action(source: Source, kind: "crawl" | "toggle" | "delete") {
    ...
    try {
      ...
      setNotice(kind === "crawl" ? `최근 기사 ${payload.fetchedCount}개 확인, 신규 ${payload.insertedCount}개 저장` : kind === "delete" ? "사이트와 저장된 기사를 삭제했습니다." : source.is_active ? "자동 수집을 중지했습니다." : "자동 수집을 다시 시작했습니다.");
      await load();
  ...
  return <div className="app-shell"><Navigation/><main className="main-area">
    <header className="topbar"><div className="mobile-brand"><span className="brand-mark">S</span><strong>signalist</strong></div><div className="live-pill"><i className="pulse-dot"/> DAILY CRAWLER</div><div className="top-actions"><Link className="secondary-button" href="/news/archive">기사 아카이브</Link><Link className="secondary-button" href="/news">본문 직접 분석</Link></div></header>
    <div className="content source-content">
      <section className="source-hero"><div><p className="eyebrow">DAILY NEWS SCRAPER</p><h1>뉴스 사이트를 돌며<br/><span>최신 기사만 저장합니다.</span></h1><p>뉴스 목록 URL에 접속해 설정한 기간 안에 발행된 기사만 추출합니다. RSS·Atom, JSON-LD와 HTML 시간 태그를 자동 판독하고 같은 기사 URL은 중복 저장하지 않습니다.</p></div><div className="pipeline-strip"><span>사이트 접속</span><b>→</b><span>발행시각 판독</span><b>→</b><span>중복 제거</span><b>→</b><span>DB 저장</span></div></section>
      ...
      <section className="source-grid">
        <form className="source-form panel" onSubmit={addSource}>
          ...
          <label>목록 페이지 수 <small>RSS는 무관</small><select value={maxPages} onChange={(event) => setMaxPages(Number(event.target.value))}>{Array.from({ length: 10 }, (_, value) => <option key={value + 1} value={value + 1}>{value + 1}페이지까지</option>)}</select></label>
          <label>수집 기간 <small>발행 시각 기준</small><select value={windowHours} onChange={(event) => setWindowHours(Number(event.target.value))}>{[12, 24, 36, 48, 72, 168].map((value) => <option key={value} value={value}>최근 {value}시간</option>)}</select></label>
          <button className="primary-button source-submit" disabled={busy === "add" || !url}>{busy === "add" ? "사이트 접속·오늘 기사 수집 중…" : "등록하고 오늘 기사 수집 →"}</button>
          <p className="source-note">페이지에 표시된 발행시각을 기준으로 설정한 기간 안의 기사만 저장합니다. 해외 매체는 현지 시간대 때문에 한국시간 날짜가 어긋나므로 48시간 이상을 권장합니다. 목록 페이지 수를 올리면 오늘 기사가 나오는 동안 다음 페이지까지 따라갑니다. RSS가 있으면 자동 사용하며 JSON-LD·HTML 시간 태그도 읽습니다. robots.txt가 금지한 경로와 로그인·유료·내부망 페이지는 제외합니다.</p>
        </form>
        ...
        <div className="source-overview panel">
          ...
          {!loading && !sources.length ? <div className="source-empty"><span>＋</span><strong>등록된 뉴스 사이트가 없습니다.</strong><p>왼쪽에서 뉴스 목록 URL을 등록하면 최신 기사를 즉시 수집하고 이후 매일 자동 실행합니다.</p></div> : <div className="source-list">{sources.map((source) => <article className="source-row" key={source.id}>
            <div className="source-main"><div><span className={`status-dot ${source.last_status === "오류" ? "failed" : source.is_active ? "active" : "paused"}`}/><strong>{source.name}</strong><small>{source.category} · {source.last_status} · 최근 {source.window_hours}시간{source.max_pages > 1 ? ` · ${source.max_pages}페이지` : ""}</small></div><a href={source.url} target="_blank" rel="noreferrer">{source.url}</a>{source.last_error && <p>{source.last_error}</p>}</div>
            <div className="source-stats"><span><small>최근 48시간 / 누적</small><strong>{source.recent_article_count} / {source.article_count}</strong></span><span><small>마지막 실행</small><strong>{when(source.last_crawled_at)}</strong></span><span><small>다음 실행</small><strong>{source.is_active ? when(source.next_crawl_at) : "중지됨"}</strong></span></div>
      ...
      <section className="article-panel panel">
        <div className="panel-title"><div><p className="eyebrow">TODAY&apos;S INGESTION</p><h2>{collectionDate || "오늘"} 저장 기사</h2></div><Link className="secondary-button" href="/news/archive">전체 아카이브 보기 →</Link></div>
        {!articles.length ? <p className="article-empty">최근 48시간 안에 저장된 기사가 아직 없습니다.</p> : <div className="article-table"><div className="article-row article-header"><span>기사</span><span>감성</span><span>유형·중요도</span><span>시장 신호</span></div>{articles.map((article) => <a className="article-row" key={article.id} href={article.canonical_url} target="_blank" rel="noreferrer"><span><strong>{article.title}</strong><small>{article.source_name} · {when(article.published_at || article.collected_at)}</small></span><span className={article.sentiment === "긍정" ? "positive" : article.sentiment === "부정" ? "negative" : "neutral"}>{article.sentiment} {article.sentiment_score}</span><span>{article.event_type} · {article.materiality}</span><span className={article.score_adjustment > 0 ? "positive" : article.score_adjustment < 0 ? "negative" : "neutral"}>{article.score_adjustment > 0 ? "+" : ""}{article.score_adjustment}</span></a>)}</div>}
      </section>
