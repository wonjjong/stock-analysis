...
`/news/sources`에서는 뉴스 사이트나 뉴스 목록 URL을 등록할 수 있습니다. 등록 시 RSS·Atom, JSON-LD, HTML의 발행일 정보를 판독해 한국시간 오늘 기사만 즉시 수집합니다. 이후 Cloudflare Worker Cron이 매시간 기한이 된 사이트를 찾아 설정된 한국시간에 하루 한 번 실행합니다. 제목·요약·발행시각·출처 URL과 경량 시장 영향 분석은 D1에 저장되며 기사 URL 기준으로 중복을 제거합니다.
...
- 현재 자동 수집기는 D1의 `news_sources`, `news_articles`, `news_crawl_runs`에 사이트·기사·실행 이력을 저장합니다. 저작권 위험을 낮추기 위해 공개 목록이 제공한 제목·요약과 원문 링크만 보관하며, 전문 재게시를 하지 않습니다.
- RSS·Atom·JSON-LD·HTML 시간 태그 순으로 발행일을 식별합니다. 로그인이 필요한 페이지, 유료 기사, 내부망 주소는 제외하고 범용 판독이 어려운 사이트는 허용 정책을 확인한 전용 어댑터를 추가합니다.
