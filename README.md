...
- 원문 뉴스도 DB에 저장합니다. `news_articles`에는 URL·제목·발행시각·해시를, `news_content_versions`에는 정제 본문과 수집시각을 버전별로 저장합니다.
- RSS·공식 API·라이선스 피드를 우선하고, 크롤링은 robots.txt와 매체 이용약관을 통과한 소스만 허용합니다. 사이트별 속도 제한·재시도·차단 스위치를 둡니다.
